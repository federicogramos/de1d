...
//SIGNAL
		...
		if(timerDelayConstant[TMR_1L_PUL]==0)//SIGUE ENTRADA Y CANCELA
			{
			if(TMR_STOPPED(TMR_1L_OFF))
				{
				if(SIGNAL==0)//ASERTIVA
					{
				...
				if(SIGNAL==1)//0 LOGICO
					{
					TMR_STOP(TMR_1L_OFF);
					}
				}
			}
		else if(timerDelayConstant[TMR_1L_OFF]==0)//DELAY CON PULSO INFINITO
			{
			if(TMR_STOPPED(TMR_1L_PUL))
				{
				if(SIGNAL==0)//ASERTIVA
					{
					TMR_START(TMR_1L_PUL);
		...
		else//delay+pulso no se calcela, dispara por estado de input
			{
			if(TMR_TIMEOUT(TMR_1L_PUL))
				{
				LED=0;
			...
			if(TMR_STOPPED(TMR_1L_OFF) && TMR_STOPPED(TMR_1L_PUL))
				{
				if(SIGNAL==0)//ASERTIVA
					{
					TMR_START(TMR_1L_OFF);
